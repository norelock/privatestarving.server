...
console.log(figlet.textSync("PrivateStarving"), "\n",
  `Server build: ${git.count()}`, `Starve.io version: ${gameServer.starveVersion}`);

const server = https.createServer({
  key: fs.readFileSync("src/data/ssl/key.pem"),
  cert: fs.readFileSync("src/data/ssl/cert.pem")
});
    ...
    try {
      if (typeof message == "string") {
        ...
        if (typeof json[0] == "string" && player == undefined) {
          ...
          var bounds = gameServer.map.spawnBiomes[Math.floor(Math.random() * gameServer.map.spawnBiomes.length)].bounds;
          gameServer.addEntity(player, { x: Utils.getRandomInt(bounds.min.x, bounds.max.x), y: Utils.getRandomInt(bounds.min.y, bounds.max.y) });
