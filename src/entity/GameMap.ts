    ...
    async initialize(gameServer: GameServer) {
        ...
        for (const mapEntity of this.entities) {
            ...
            if (config.debug.drawMap) {
                const entity = gameServer.addEntity(new Entity(gameServer, EntityType.list.findId(36)!), mapEntity, "circle:40", { isSensor: true });
                if (mapEntity.body === undefined) {
                    mapEntity.body = entity.body;
                }
            }
        }
