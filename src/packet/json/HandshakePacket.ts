...
export class HandshakePacket implements JsonPacket {
    ...
    build(): object {
        return [this.nickname, this.width, this.height, this.clientVersion, "", "", 0, this.skin.skinId, this.skin.accessoryId, this.skin.bagId, this.skin.bookId, this.skin.lootBox, this.skin.deadBox, this.userId];
    }
...
export class HandshakeResponse implements JsonPacket {
    ...
    build(): object {
        return [3, 1, 0, this.player.body.position.x, this.players.sort(x => x.points).map(x => x.asLeaderBoard()), this.night ? 1 : 0, 0, 1000, [], this.player.id, this.player.body.position.y, this.maxPlayers + 1, 0, 0, [], this.time, 0, [], 0, this.mapSeed];
    }
